...
/*   Updated: 2022/04/12 19:18:35 by agenoves         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_status(t_philo *philo)
{
	...
	if (philo->last_meal >= philo->to_die)
	{
		philo->is_dead = 1;
		return (printf("Philo X is dead"));
	}
	if (philo->to_eat >= philo->to_die)
	{
		philo->is_dead = 1;
		return (printf("Philo X is dead"));
	}
...
int	ft_create_thread(t_philo *philo)
{
	...
	i = -1;
	th = malloc(sizeof(pthread_t) * philo->num_philo);
	if (!th)
		return (1);
	while (++i < philo->num_philo)
		if (pthread_create(&th[i], NULL, ft_routine, philo) != 0)
